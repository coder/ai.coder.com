...
locals {
  ...
  policy_name = var.policy_name == "" ? "${var.cluster_name}-crt-mgr-${data.aws_region.this.region}" : var.policy_name
  role_name   = var.role_name == "" ? "${var.cluster_name}-crt-mgr-${data.aws_region.this.region}" : var.role_name
}
